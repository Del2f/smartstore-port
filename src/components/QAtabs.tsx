    ...
    <div id="inquiry" className="panel-body">
      <ul className="nav nav-tabs">
        {data.map(item => (
          <li 
          key={item.id}
          className={index === item.id ? 'nav-item active' : 'nav-item'}
          onClick={() => setIndex(item.id)}>
            <p className={index === item.id ? 'text-number text-number-active' : 'text-number'}>{item.alarm}</p>
            <button type="button" className={index === item.id ? 'nav-link active ' + item.class + 'click'  : 'nav-link ' + item.class}>{item.title}</button>
          </li>
        ))}
      </ul>
      {data.filter(item => index === item.id).map(item => (
        <div className="tab-pane qna-content">
          ...
          <div className="qna-btn-wrap">
            <button className="qna-btn">{item.btn}<ChevronRight className="qna-btn-icon"/></button>
          </div>
        </div>
      ))}
      </div>
